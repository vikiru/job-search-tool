...
import { userContactSchema, type UserContactData, type UserContactUpdate } from '@/entities/user/schemas';
...
function validateContactInput(input: unknown): Result<UserContactUpdate> {
  const parsed = userContactSchema.safeParse(input);
  return parsed.success ? success(parsed.data) : error('Invalid contact information.');
}

function normalizeLink(link: UserContactData['links'][number]): UserContactData['links'][number] | null {
